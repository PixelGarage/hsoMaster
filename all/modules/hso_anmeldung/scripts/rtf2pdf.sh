...
PATH=/bin:/usr/local/bin:/bin:/usr/bin:$PATH
...
case $PAPER in
    ?*)
	$(dirname $(readlink -f "$0"))/Ted --printToFilePaper "$1" --setProperty leftMargin 25mm --setProperty afmDirectory $(dirname $(readlink -f "$0"))/afm /tmp/$$.ps $PAPER
